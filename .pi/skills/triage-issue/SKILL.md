...
Use `bash` and `read` tools to deeply investigate the codebase. Your goal is to find:
...
- `read` source files to trace flows
- `git log --oneline -10 -- <file>` to see recent changes
- Look at existing tests for the area
- Compare with similar patterns that work correctly
